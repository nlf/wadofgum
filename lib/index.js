...
var createFactory = function (options) {
    ...
    Factory.extend = function (factories) {
        ...
        var opts = Joi.validate(factories, Schema.factories);
        if (opts.error) {
            throw new Error('Invalid factory');
        }

        for (var i = 0, il = opts.value.length; i < il; ++i) {
            var factory = opts.value[i];
    ...
    Factory.register = function (plugins) {
        ...
        var opts = Joi.validate(plugins, Schema.plugins.min(1));
        if (opts.error) {
            throw new Error('Invalid plugin');
        }

        for (var i = 0, il = opts.value.length; i < il; ++i) {
            var plugin = opts.value[i];
            var pluginOpts = typeof plugin === 'object' ? plugin.options : {};
            var fn = typeof plugin === 'function' ? plugin : (typeof plugin.register === 'function' ? plugin.register : plugin.register.register);

            fn.call(null, Factory, pluginOpts);
        }
